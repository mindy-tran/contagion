...
class AtRisk(Individual):
    ...
    def hello(self, visitor: "Individual") -> bool:
        """True means 'welcome' and False means 'go away'"""
        # better: return (visitor.row, visitor.col) in self.neighbors
        for i in range(len(self.neighbors)):
            if visitor == self.neighbors[i]:
                return True
        return False
...
class Wanderer(Individual):
    """Travelers ignored warnings to keep close to home and traveled widely"""
    def __init__(self, region: "Population", row: int, col: int):
    ...
    def social_behavior(self):
        ...
        if random.random() < self.P_Visit:
            row_addr = random.choice(self.rows)
            col_addr = random.choice(self.col)
            addr = (row_addr, col_addr)
            person = self.region.visit(addr)
            if person.hello(self):
                person.meet(self)
